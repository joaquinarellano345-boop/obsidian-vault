---
source_file: "tests/test_semantic_id_remap_root.py"
type: "code"
community: "_semantic_id_remap"
location: "L55"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/_semantic_id_remap
---

# test_semantic_id_remap_is_idempotent_when_stem_contains_legacy_stem()

## Connections
- [[A file whose parent dir name equals its stem (.claudeCLAUDE.md -     canonical]] - `rationale_for` [EXTRACTED]
- [[_semantic_id_remap()]] - `calls` [EXTRACTED]
- [[test_semantic_id_remap_root.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/_semantic_id_remap