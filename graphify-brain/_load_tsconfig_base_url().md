---
source_file: "graphify/extractors/resolution.py"
type: "code"
community: "extract_astro"
location: "L223"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/extract_astro
---

# _load_tsconfig_base_url()

## Connections
- [[Path_30]] - `references` [EXTRACTED]
- [[_find_js_config()]] - `calls` [EXTRACTED]
- [[_read_json_config()]] - `calls` [EXTRACTED]
- [[_resolve_js_module_path()]] - `calls` [EXTRACTED]
- [[`compilerOptions.baseUrl` of the nearest config, as an absolute directory.]] - `rationale_for` [EXTRACTED]
- [[extract.py]] - `imports` [EXTRACTED]
- [[extract_astro()]] - `calls` [EXTRACTED]
- [[extract_svelte()]] - `calls` [EXTRACTED]
- [[extract_vue()]] - `calls` [EXTRACTED]
- [[resolution.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/extract_astro