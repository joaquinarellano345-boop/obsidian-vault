---
source_file: "graphify/extract.py"
type: "code"
community: "test_languages.py"
location: "L1663"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/test_languagespy
---

# extract_cpp()

## Connections
- [[Extract functions, classes, and includes from a .cpp.cc.cxx.hpp file.]] - `rationale_for` [EXTRACTED]
- [[Path_10]] - `references` [EXTRACTED]
- [[_extract_generic()]] - `calls` [EXTRACTED]
- [[_get_extractor()]] - `indirect_call` [INFERRED]
- [[extract.py]] - `indirect_call` [INFERRED]
- [[test_cpp_class_inherits_edge()]] - `calls` [EXTRACTED]
- [[test_cpp_field_and_template_argument_contexts()]] - `calls` [EXTRACTED]
- [[test_cpp_finds_class()]] - `calls` [EXTRACTED]
- [[test_cpp_finds_includes()]] - `calls` [EXTRACTED]
- [[test_cpp_finds_methods()]] - `calls` [EXTRACTED]
- [[test_cpp_generic_parents_include_type_argument_references()]] - `calls` [EXTRACTED]
- [[test_cpp_import_edges_have_import_context()]] - `calls` [EXTRACTED]
- [[test_cpp_method_parameter_and_return_type_contexts()]] - `calls` [EXTRACTED]
- [[test_cpp_no_error()]] - `calls` [EXTRACTED]
- [[test_cpp_struct_inherits_edge()]] - `calls` [EXTRACTED]
- [[test_cuda_finds_includes()]] - `calls` [EXTRACTED]
- [[test_cuda_finds_kernel_and_device_functions()]] - `calls` [EXTRACTED]
- [[test_cuda_finds_struct()]] - `calls` [EXTRACTED]
- [[test_cuda_host_call_edges()]] - `calls` [EXTRACTED]
- [[test_cuda_no_error()]] - `calls` [EXTRACTED]
- [[test_languages.py]] - `imports` [EXTRACTED]
- [[test_metal_finds_kernel_function_and_struct()]] - `calls` [EXTRACTED]
- [[test_metal_no_error()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/test_languagespy